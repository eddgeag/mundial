...
# Monte Carlo V6D: KO dinámico + bracket oficial FIFA Annex C
# ============================================================
#
...
#
# Salidas:
#   modelos_v6_feature_engineered/MONTE_CARLO_V6D_OFFICIAL_ANNEXC/
#
# ============================================================
...
mc_out_dir <- file.path(out_dir, "MONTE_CARLO_V6D_OFFICIAL_ANNEXC")
...
# Probabilidades para fase de grupos: usualmente ensemble.
GROUP_PROB_SOURCE <- "ensemble"

# Probabilidades para KO: ensemble calculado dinámicamente desde modelos.
KO_PROB_SOURCE <- "ensemble"
...
# Archivos
pred_file <- file.path(out_dir, "fixtures_2026_predictions_all_models_v6.csv")
train_v6_file <- file.path(data_dir, "training_v6_feature_engineered.csv")
fixtures_v6_file <- file.path(data_dir, "fixtures_2026_v6_feature_engineered.csv")

metadata_file <- file.path(out_dir, "model_metadata_v6.rds")
model_multinom_file <- file.path(out_dir, "model_multinomial_v6.rds")
model_rf_file <- file.path(out_dir, "model_random_forest_v6_tuned.rds")
model_xgb_file <- file.path(out_dir, "model_xgboost_v6_tuned.xgb")

cat("\n==============================\n")
cat("MONTE CARLO V6D OFFICIAL ANNEX C CONFIG\n")
cat("==============================\n")
cat("N_SIM:", N_SIM, "\n")
...
model_multinom <- readRDS(model_multinom_file)
model_rf <- readRDS(model_rf_file)
model_xgb <- xgb.load(model_xgb_file)
...
# Pesos de ensemble desde metadata.
if (!is.null(metadata$ensemble_best_weights)) {
  w_rf_best <- metadata$ensemble_best_weights$w_rf[1]
  w_multinom_best <- metadata$ensemble_best_weights$w_multinom[1]
  w_xgb_best <- metadata$ensemble_best_weights$w_xgb[1]
} else {
  warning("No encuentro ensemble_best_weights en metadata. Uso pesos por defecto.")
  w_rf_best <- 0.45
  w_multinom_best <- 0.15
  w_xgb_best <- 0.40
}
...
# Levels categóricos desde training.
cat_levels <- list()
...
# ------------------------------------------------------------
# 3. Seleccionar probabilidades de grupos
# ------------------------------------------------------------
...
if (GROUP_PROB_SOURCE == "ensemble") {
  prob_cols <- c("ensemble_p_team_A_win", "ensemble_p_draw", "ensemble_p_team_B_win")
} else if (GROUP_PROB_SOURCE == "rf") {
  prob_cols <- c("rf_p_team_A_win", "rf_p_draw", "rf_p_team_B_win")
} else if (GROUP_PROB_SOURCE == "xgb") {
  prob_cols <- c("xgb_p_team_A_win", "xgb_p_draw", "xgb_p_team_B_win")
} else if (GROUP_PROB_SOURCE == "multinomial") {
  prob_cols <- c("multinomial_p_team_A_win", "multinomial_p_draw", "multinomial_p_team_B_win")
} else {
  stop("GROUP_PROB_SOURCE no reconocido: ", GROUP_PROB_SOURCE)
}

missing_probs <- setdiff(prob_cols, names(fixtures_pred))
if (length(missing_probs) > 0) {
  stop("Faltan columnas de probabilidad para grupos: ", paste(missing_probs, collapse = ", "))
}
...
safe_fwrite(
  team_profile,
  file.path(mc_out_dir, "mc_team_profile_v6D.csv")
)
...
predict_match_probs_dynamic <- function(team_A, team_B) {
  ...
  # A veces predict multinom devuelve vector nombrado.
  ...
  # XGB
  x_new <- model.matrix(formula_x_v6, data = new_row)[, -1, drop = FALSE]
  x_new <- align_model_matrix(x_new, xgb_feature_names)
  ...
  # Ensemble
  prob_ens <- data.table(
    HomeWin = w_rf_best * prob_rf$HomeWin + w_multinom_best * prob_mult$HomeWin + w_xgb_best * prob_xgb$HomeWin,
    Draw    = w_rf_best * prob_rf$Draw    + w_multinom_best * prob_mult$Draw    + w_xgb_best * prob_xgb$Draw,
    AwayWin = w_rf_best * prob_rf$AwayWin + w_multinom_best * prob_mult$AwayWin + w_xgb_best * prob_xgb$AwayWin
  )
  ...
  if (KO_PROB_SOURCE == "ensemble") {
    out[, `:=`(
      p_A_win = ensemble_HomeWin,
      p_draw = ensemble_Draw,
      p_B_win = ensemble_AwayWin
    )]
  } else if (KO_PROB_SOURCE == "rf") {
    out[, `:=`(
      p_A_win = rf_HomeWin,
      p_draw = rf_Draw,
      p_B_win = rf_AwayWin
    )]
  } else if (KO_PROB_SOURCE == "xgb") {
    out[, `:=`(
      p_A_win = xgb_HomeWin,
      p_draw = xgb_Draw,
      p_B_win = xgb_AwayWin
    )]
  } else if (KO_PROB_SOURCE == "multinomial") {
    out[, `:=`(
      p_A_win = multinom_HomeWin,
      p_draw = multinom_Draw,
      p_B_win = multinom_AwayWin
    )]
  } else {
    stop("KO_PROB_SOURCE no reconocido: ", KO_PROB_SOURCE)
  }
...
safe_fwrite(
  ko_prob_table,
  file.path(mc_out_dir, "mc_KO_dynamic_prob_precomputed_all_pairs_v6D.csv")
)
...
cat("\n==============================\n")
cat("INICIANDO SIMULACIONES V6D\n")
cat("==============================\n")
...
safe_fwrite(
  stage_probs,
  file.path(mc_out_dir, "mc_team_stage_probabilities_v6D_official_annexC.csv")
)
...
safe_fwrite(
  position_probs,
  file.path(mc_out_dir, "mc_group_position_probabilities_v6D_official_annexC.csv")
)
...
safe_fwrite(
  champion_probs,
  file.path(mc_out_dir, "mc_simulation_champions_v6D_official_annexC.csv")
)
...
safe_fwrite(
  group_summary,
  file.path(mc_out_dir, "mc_group_summary_v6D_official_annexC.csv")
)
...
if (length(ko_match_log_sample) > 0) {
  ...
  safe_fwrite(
    ko_log_dt,
    file.path(mc_out_dir, "mc_KO_match_log_sample_first5_v6D.csv")
  )
}
...
#   safe_fwrite(
#     ko_cache_dt,
#     file.path(mc_out_dir, "mc_KO_dynamic_prob_cache_v6D.csv")
#   )
# }
...
metadata_out <- list(
  ...
  ensemble_weights = data.table(
    w_rf = w_rf_best,
    w_multinom = w_multinom_best,
    w_xgb = w_xgb_best
  ),
  note = "V6D builds dynamic KO match features and uses the official FIFA Annex C table for the 495 third-place combinations and official knockout bracket pairings from Articles 12.6-12.11."
)

saveRDS(
  metadata_out,
  file.path(mc_out_dir, "mc_simulation_metadata_v6D_official_annexC.rds")
)
...
cat("\n==============================\n")
cat("MONTE CARLO V6D COMPLETADO\n")
cat("==============================\n")
